...
export default function DashboardClient({ tickets }: DashboardClientProps) {
  ...
  const [sortKey, setSortKey] = useState<keyof Ticket>("ticket_no");
  ...
  const fetchData = async () => {
    ...
    const { data: fresh, error } = await supabaseClient
      .from("birthdayparty_prize_tickets")
      .select("*")
      .order("ticket_no");
  ...
  useEffect(() => {
    const channel = supabaseClient
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "birthdayparty_prize_tickets" },
        (payload) => {
